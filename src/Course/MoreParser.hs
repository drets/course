...
-- | Parses the given input and returns the result. The remaining input is ignored.
...
-- | Write a function that applies the given parser, then parses 0 or more spaces, then produces the
-- result of the original parser.
--
...
tok p = p >>= \x -> spaces >>= \_ -> P $ \s -> Result s x
...
-- | Write a function that applies the first parser, runs the third parser keeping the result, then
-- runs the second parser and produces the obtained result.
--
-- /Tip:/ Use the monad instance.
--
...
-- True
hex :: Parser Char
hex = replicateA 4 (satisfy isHexDigit) >>=
      \xs -> case readHex xs of
        Full x -> pure (chr x)
        Empty  -> failed

-- | Write a function that parses the character 'u' followed by 4 hex digits and return the
-- character value.
--
-- /Tip:/ Use `is` and `hex`.
--
...
-- | Write a function that produces a non-empty list of values coming off the given parser (which
-- must succeed at least once), separated by the second given parser.
--
...
sepby1 p sep = liftM2 (:.) p (list (sep >> p))

-- | Write a function that produces a list of values coming off the given parser, separated by the
-- second given parser.
--
-- /Tip:/ Use `sepby1` and `|||`.
--
...
-- True
eof :: Parser ()
eof = P $ \s -> if isEmpty s
                  then Result s ()
                  else ErrorResult $ ExpectedEof s
...
satisfyAll xs = P $ \s -> let (y :. ys) = s
                          in if isErrorResult (parse eof s) && and (sequence xs y)
                               then Result ys y
                               else ErrorResult $ UnexpectedChar y
...
satisfyAny xs = P $ \s -> let (y :. ys) = s
                          in if isErrorResult (parse eof s) && or (sequence xs y)
                               then Result ys y
                               else ErrorResult $ UnexpectedChar y
...
betweenSepbyComma x y p = betweenCharTok x y $ sepby p (charTok ',')
