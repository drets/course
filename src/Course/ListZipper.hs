...
findLeft f (ListZipper l x r) = case break f l of
  (_, Nil)       -> IsNotZ
  (r', x' :. l') -> IsZ $ ListZipper l' x' (r' ++ x :. Nil ++ r)
...
findRight f (ListZipper l x r) = case break f r of
  (_, Nil)       -> IsNotZ
  (l', x' :. r') -> IsZ $ ListZipper (reverse l' ++ x :. Nil ++ l) x' r'
...
moveLeftLoop ::
  ListZipper a
  -> ListZipper a
moveLeftLoop lz@(ListZipper l x r) =
  let (y :. ys) = l
      (z :. zs) = reverse r
  in if hasLeft lz
       then ListZipper ys y (x :. r)
       else ListZipper (zs ++ x :. Nil) z Nil
...
moveRightLoop ::
  ListZipper a
  -> ListZipper a
moveRightLoop ls@(ListZipper l x r) =
  let (y :. ys) = reverse l
      (z :. zs) = r
  in if hasRight ls
       then ListZipper (x :. l) z zs
       else ListZipper Nil y (ys ++ x :. Nil)
...
moveLeft ::
  ListZipper a
  -> MaybeListZipper a
moveLeft lz | hasLeft lz = IsZ $ moveLeftLoop lz
moveLeft _               = IsNotZ
...
moveRight ::
  ListZipper a
  -> MaybeListZipper a
moveRight lz | hasRight lz = IsZ $ moveRightLoop lz
moveRight _                = IsNotZ
...
swapLeft ::
  ListZipper a
  -> MaybeListZipper a
swapLeft lz@(ListZipper l x r) =
  let (y :. ys) = l
  in if hasLeft lz
       then IsZ $ ListZipper (x :. ys) y r
       else IsNotZ
...
swapRight ::
  ListZipper a
  -> MaybeListZipper a
swapRight lz@(ListZipper l x r) =
  let (z :. zs) = r
  in if hasRight lz
       then IsZ $ ListZipper l z (x :. zs)
       else IsNotZ
...
nth ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
nth n lz = moveLeftN (index lz - n) lz
...
-- prop> rights (end lz) == Nil
  ...
  where
    (IsZ lz') = nth (length (toList lz) - 1) lz
...
deletePullLeft ::
  ListZipper a
  -> MaybeListZipper a
deletePullLeft lz = case swapLeft lz of
  (IsZ (ListZipper (_ :. ys) x r)) -> IsZ $ ListZipper ys x r
  _                                -> IsNotZ
...
deletePullRight ::
  ListZipper a
  -> MaybeListZipper a
deletePullRight lz = case swapRight lz of
  (IsZ (ListZipper l x (_ :. zs))) -> IsZ $ ListZipper l x zs
  _                                -> IsNotZ
...
-- /Tip:/ Use `zipWith`
  ListZipper l x r <*> ListZipper l' x' r' =
    ListZipper (zipWith id l l') (x x') (zipWith id r r')
...
instance Extend ListZipper where
  f <<= lz =
    ListZipper (f <$> unfoldr (toOptionalPair . moveLeft) lz) (f lz) (f <$> unfoldr (toOptionalPair . moveRight) lz)
        where
          toOptionalPair :: MaybeListZipper a -> Optional (ListZipper a, ListZipper a)
          toOptionalPair IsNotZ = Empty
          toOptionalPair (IsZ a) = Full (a, a)
