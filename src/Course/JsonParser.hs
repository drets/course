...
jsonString = between (is '"') (is '"') (list char)
  where
    char :: Parser Char
    char = (is '\\' >> (hexu ||| esc)) ||| satisfy (\x -> x /= '"' && x /= '\\')

    esc :: Parser Char
    esc = P $ \(c :. cs) -> case toSpecialCharacter c of
      Full a -> Result cs (fromSpecialCharacter a)
      _      -> ErrorResult $ UnexpectedChar c
