...
mapParser :: (a -> b)
          -> Parser a
          -> Parser b
mapParser f p = P $ \s -> case parse p s of
...
satisfy ::
  (Char -> Bool)
  -> Parser Char
satisfy p = bindParser (\x -> if p x
        then return x
        else unexpectedCharParser x
        ) character
...
sequenceParser = foldRight (\p px -> (:.) <$> p <*> px) (pure Nil)
...
instance Functor Parser where
  (<$>) ::
    ...
    -> Parser a
    -> Parser b
  f <$> p = bindParser (valueParser . f) p
...
instance Applicative Parser where
  ...
  (<*>) ::
    ...
    -> Parser a
    -> Parser b
  m <*> p = bindParser (\f -> bindParser (valueParser . f) p) m
